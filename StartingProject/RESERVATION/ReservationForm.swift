...
struct ReservationForm: View {
    ...
    var body: some View {
        VStack {
            ...
            .alert(isPresented: $showFormInvalidMessage, content: {
                Alert(title: Text("ERROR"), message: Text(errorMessage), dismissButton: .cancel(Text("OK")))
    ...
    private func validateForm() {
        ...
        let phoneIsValid = isValid(email: customerPhoneNumber)
    ...
    func isValid(phone:String) -> Bool {
        ...
        let emailValidationRegex = "^[\\p{L}0-9!#$%&'*+\\/=?^_`{|}~-][\\p{L}0-9.!#$%&'*+\\/=?^_`{|}~-]{0,63}@[\\p{L}0-9-]+(?:\\.[\\p{L}0-9-]{2,7})*$"
        let emailValidationPredicate = NSPredicate(format: "SELF MATCHES %@", emailValidationRegex)
        return emailValidationPredicate.evaluate(with: phone)
    }
